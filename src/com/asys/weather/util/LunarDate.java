...
public class LunarDate {
	...
	// base date for lunar is 1900-01-31
	private static final long BASE_MILLISECONDS = -2206425600000L;
	...
	public static Lunar getLunarDate(Date date) {
		int iYear, iMonth, iDay, leapMonth;
		boolean leap = false;
		int offsetDay = (int) ((date.getTime() - BASE_MILLISECONDS) / MILLISECONDS_ONE_DAY);
		// 用offset减去每农历年的天数
		// 计算当天是农历第几天
		// i最终结果是农历的年份
		// offset是当年的第几天
		for (iYear = 1900; iYear < 2050 && offsetDay > 0; iYear++) {
			int daysOfYear = yearDays(iYear);
			if (offsetDay < daysOfYear) {
				break;
			}
		...
		// 用当年的天数offsetDay, 逐个减去每月（农历）的天数，求出当天是本月的第几天
		int daysOfMonth = 0;
		for (iMonth = 1; iMonth < 13 && offsetDay > 0; iMonth++) {
			// 闰月
		...
		if (offsetDay == 0 && leapMonth > 0 && iMonth == leapMonth + 1) {
			if (leap) {
				leap = false;
			} else {
				leap = true;
		...
		iDay = offsetDay + 1;
		return new Lunar(cyclical(iYear), animalsYear(iYear), (leap ? "闰" : "") + chineseNumber[iMonth - 1], getChinaDayString(iDay));
	}
}
