...
public class WeekPage {
	...
	public void loadScene(Scene scene, Font font, VertexBufferObjectManager vertexBufferObjectManager, WeatherInfo weather) {
		date1 = new Text(-Config.CAMERA_WIDTH, 120, font, "", 40, vertexBufferObjectManager);
		scene.attachChild(date1);

		date2 = new Text(-Config.CAMERA_WIDTH, 160, font, "", 40, vertexBufferObjectManager);
		scene.attachChild(date2);

		date3 = new Text(-Config.CAMERA_WIDTH, 200, font, "", 40, vertexBufferObjectManager);
		scene.attachChild(date3);
		date4 = new Text(-Config.CAMERA_WIDTH, 240, font, "", 40, vertexBufferObjectManager);
		scene.attachChild(date4);
		date5 = new Text(-Config.CAMERA_WIDTH, 280, font, "", 40, vertexBufferObjectManager);
		scene.attachChild(date5);

		date6 = new Text(-Config.CAMERA_WIDTH, 320, font, "", 40, vertexBufferObjectManager);
